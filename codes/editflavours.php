...
if(isset($_POST['add'])){
    ...
    $url = $_POST['url'];
    $price = $_POST['price'];
    $sql ="INSERT INTO `cakecat`(`catid`, `name`, `url`, `price`) VALUES (?,?,?,?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('issi', $catid, $name, $url, $price);
...
if(isset($_POST['update'])){
    ...
    $url = $_POST['url'];
    $price = $_POST['price'];
    $sql ="UPDATE `cakecat` SET `name` = ?, `url` = ?, `price` = ? WHERE `catid` = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('ssii',$name,$url,$price,$catid);
...
if(isset($_POST['delete'])){
    $catid = $_POST['catid'];
    $sql ="DELETE FROM `cakecat` WHERE `catid` = ?";
    $stmt = $conn->prepare($sql);
    ...
    <div class="cat" style="text-align:center;">
        <button onclick="toggleAddForm()" class="btn">ADD</button>
            ...
            <pre>
                ...
                <input type="text" name="catid" placeholder="Enter ID"><br>
                ...
                <button type="submit" name="add">ADD to cake flavours</button><br>
            </pre>
            </form>
        ...
        <button onclick="toggleDeleteForm()" class="btn" >DELETE</button>
            ...
            <pre>
                <input type="text" name="catid" placeholder="Enter ID"><br>
                <button type="submit" name="delete">DELETE cake flavours</button><br>
            </pre>
            </form>
